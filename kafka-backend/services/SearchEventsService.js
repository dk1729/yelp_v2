...
function handle_request(msg, callback){   
  console.log("Inside search events kafka backend");
  console.log(msg);
  
  //main logic
  var ans = []
  event_details.find({event_name:new RegExp(msg.term,'i')}, function(err, result){
    console.log("Hola, I got the result, señor")
    if(err){
      console.log("Err", err)
      callback(null, {code:500, message:"Some error occured"})
    }
    else if(result.length === 0){
      callback(null, {code:204, message:"Some error occured"})
    }
    else{
      console.log(result)
      let main_count=0;
  ...
  //main logic ends
  console.log("after callback of search events");
};
